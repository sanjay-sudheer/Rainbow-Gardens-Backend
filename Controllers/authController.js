...
const getAddCartPlantDetails = async (req, res, email) => {
  try {
    ...
    const plantDetails = products.map(product => ({
      Eno: email, // Changed from { S: email } to just email for clarity
      Pno: product.Pno,
...
const addPlantDetailsToCart = async (plantDetailsArray) => {
  try {
    ...
    const items = plantDetailsArray.map(plantDetails => ({
      PutRequest: {
        Item: {
          Pno: plantDetails.Pno,
          plantName: plantDetails.plantName,
          plantPrice: plantDetails.plantPrice.toString(), // Ensure price is a string
    ...
    const params = {
      RequestItems: {
        [process.env.DYNAMODB_TABLE_CARTADD]: items,
      },
    };
...
module.exports = {
  getAddCartPlantDetails,
  getaddcartPlantDetailsByPno,
  addPlantDetailsToCart,
};
